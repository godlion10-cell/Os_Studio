...
@app.route('/api/radar')
def get_radar():
    # 1. 분석팀장 뇌수술: '블로그'라는 단어 자체를 키워드에서 금지시킵니다.
    prompt = f"""
    당신은 대한민국 1타 트렌드 분석가입니다. 블로그 방문자를 폭발시킬 수 있는 '다양한 분야'의 트렌드 키워드를 10개 추출하세요.
    
    ⚠️ [절대 금지사항]: '블로그 수익', '제휴 마케팅', '글쓰기' 같은 블로그 운영 자체에 대한 키워드는 단 1개도 넣지 마세요!
    ✅ [필수 포함 분야]: IT/테크(스마트폰, AI), 부동산/경제, 2026년 정부 정책/지원금, 생활/건강, 사회 핫이슈.
    
    1. [search] (검색 유입형): 사람들이 네이버에 직접 타이핑해서 검색하는 정보성 (예: "2026년 청년도약계좌 조건", "갤럭시 S26 울트라 단점") - 5개
    2. [home] (홈판형): 썸네일만 보고 홀린듯 클릭하는 도파민/호기심 유발형 (예: "강남 꼬마빌딩 반토막 충격", "의사들이 절대 안 먹는 영양제") - 5개
    ...
    try:
        ...
        print(f"레이더 엔진 에러: {e}")
        return jsonify({
            "search": ["2026년 신생아 특례대출", "chatGPT 4.5 실전 활용법", "아이폰 18 디자인 유출", "근로장려금 신청 방법", "미국 배당주 ETF 추천"],
            "home": ["한국인 90%가 모르는 세금 환급", "평생 후회하는 3가지 소비 습관", "지금 안 사면 품절되는 다이소템", "은퇴 후 월 300 파이프라인", "당뇨를 부르는 최악의 아침식사"]
        })
